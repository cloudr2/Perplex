...
public class UICamera : MonoBehaviour
{
	...
	/// <summary>
	/// Delay for the OnClickOnce
	/// </summary>
	public float delay4NextClick = 0.5f;
	/// <summary>
	...
	float intervalClickTime = 0f; //Use for OnClickOnce to reset values
	
	Dictionary<GameObject, int> clickCountById = new Dictionary<GameObject, int>(); //Last touchId count
	...
	int GetClickAmount(GameObject currentGO)
	{
		if(currentGO == null)return 0;
		
		int currentCount = 1;
		
		if(clickCountById.TryGetValue(currentGO, out currentCount))
		{
			int auxCount = (currentCount + 1);
			clickCountById[currentGO] = auxCount;
			return auxCount;
		}
		else
			clickCountById.Add(currentGO, 1);

		return 1;
	}
	
	public Dictionary<int, MouseOrTouch> MouseOrTouchDic {
		get { return mTouches ;}
	}	
	...
	void FixedUpdate ()
	{
		...
		if(((clickCount > 0) && (Time.fixedTime - intervalClickTime) > delay4NextClick))
		{
			clickCountById.Clear();
			clickCount = 0;
		}
	}
			...
			if (currentTouch.pressed != null)
			{
					...
					(currentTouch.clickNotification != ClickNotification.None && currentTouch.totalDelta.magnitude < drag))
				{
					...
					if (currentTouch.clickNotification != ClickNotification.None)
					{
						...
						clickCount = GetClickAmount(currentTouch.current);
						
						if(clickCount == 1)Notify(currentTouch.pressed, "OnClickOnce", null);
						...
						intervalClickTime = Time.fixedTime;
