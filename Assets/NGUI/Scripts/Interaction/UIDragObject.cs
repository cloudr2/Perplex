...
public class UIDragObject : IgnoreTimeScale
{
	...
	{
		get { return hasBeenDragged;  }
		set { hasBeenDragged = value; }
	}
	
	public bool adjustToScreenResolution;
	...
	public float deltaAdjustment = 1000f;
	public Vector3 MLastPosition{ set{this.mLastPos = value;}}
	Plane mPlane;
	Vector3 mLastPos;
	UIPanel mPanel;
	bool mPressed = false;
	...
	float mScroll = 0f;
	Bounds mBounds;
	int scaleForMaxScreen = -320;
	int maxScreenWidth    = 960;
	int maxScreenHeight   = 640;
	bool hasBeenDragged;
	/// <summary>
	...
	private Vector3 AdjustScaleToScreenResolution(Vector3 scale) 
	{	
		// get the current screen size
		int screenWidth = Screen.width;
		int screenHeight = Screen.height;
		
		//adjust the scale to move the object following the cursor
		float adjustedScaleY = screenHeight * scaleForMaxScreen / maxScreenHeight;
		float adjustedScaleX = screenWidth * scaleForMaxScreen / maxScreenWidth;
		
		//replace the old scale with the new adjusted scale
		Vector3 vectorScaleAdjusted = new Vector3(0,0,1);
		vectorScaleAdjusted.y = adjustedScaleY;
		vectorScaleAdjusted.x = adjustedScaleX;
		return vectorScaleAdjusted;
	...
	{	
		if (enabled && NGUITools.GetActive(gameObject) && target != null)
		{
			...
			if(Input.touchCount >= 2)
				ray = UICamera.currentCamera.ScreenPointToRay((Vector3)Input.GetTouch(1).position );
			else
				ray = UICamera.currentCamera.ScreenPointToRay((Vector3)Input.GetTouch(0).position );
			float dist = 0f;
